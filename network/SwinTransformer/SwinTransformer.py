...
class SwinTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
        ...
        # if self.ape:
        #     self.absolute_pos_embed = nn.Parameter(torch.zeros(1, num_patches, embed_dim))# (1,56*56,96)
        #     trunc_normal_(self.absolute_pos_embed, std=.02)
        
        self.ape = False
        if self.ape:
            # local_ape
            # self.local_ape = nn.Parameter(torch.zeros(1, 96, 56, 56))
            self.absolute_pos_embed = nn.Parameter(torch.zeros(1,  96, 56, 56))# (1,56*56,96)
            trunc_normal_(self.absolute_pos_embed, std=.02)
    ...
    def forward_features(self, x, local_ape=None):
        x = self.patch_embed(x)
        if self.ape:
            # print(local_ape.shape)
            # self.local_ape = F.interpolate(self.local_ape,size=(28,28),mode='nearest')

            # print(self.absolute_pos_embed.shape)
            with torch.no_grad():
                self.absolute_pos_embed[:,:,14:42,14:42] = local_ape[:,:,::2,::2]
                absolute_pos_embed = self.absolute_pos_embed.view(1,self.embed_dim,-1).permute(0,2,1)
                # x = x + self.absolute_pos_embed.view(1,self.embed_dim,-1).permute(0,2,1)
