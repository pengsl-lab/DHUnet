...
def deit_small_patch16_224(pretrained=False, **kwargs):
    ...
    if pretrained:
        ckpt = torch.load('./pretrained_ckpt/deit_small_patch16_224-cd65a155.pth')
        model.load_state_dict(ckpt['model'], strict=False)
    
    pe = model.pos_embed[:, 1:, :].detach()
    
    # RuntimeError: The size of tensor a (196) must match the size of tensor b (192) at non-singleton dimension 1
    # pe = pe.transpose(-1, -2)
